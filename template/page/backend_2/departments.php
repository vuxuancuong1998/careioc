...
<script>
	...
	$(document).ready(function(){
			...
			Swal.fire({
			  ...
			  if (result.isConfirmed) {
				  var id = button.attr("data-id");
					$.ajax({
						type: "POST",
						url: "<?php echo XC_URL;?>/api/deletedepart",
						data: {id:id},
						dataType: "json",
						cache: false,
						success: function(data)
		...
		$("#btn-save").on("click",function()
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo XC_URL;?>/api/adddepart",
				data: fd,
				dataType: "json",
				cache: false,
...
<div class="bg-body-light">
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Danh sách phòng ban</h1>
            <button type="button" class="btn btn-alt-success my-2" id="btn-add-cat">
                <i class="fa fa-fw fa-plus mr-1"></i> Thêm phòng ban
            </button>
        </div>
   </div>
</div>
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
				...
				<?php
					...
					<tr>
                        ...
                        <td class="font-w600"><?php echo $d->depart_name;?></td>
						<td></td>
						<td class="text-center">
                            <div class="btn-group">
                                <button type="button" data-id="<?php echo $d->id;?>" data-name="<?php echo $d->depart_name;?>" class="btn btn-sm btn-primary js-tooltip-enabled btn-edit-meta" data-toggle="tooltip" title="" data-original-title="Meta">
                                    <i class="fa fa-atom"></i>
                                </button>
								<button type="button" data-id="<?php echo $d->id;?>" data-name="<?php echo $d->depart_name;?>" class="btn btn-sm btn-primary js-tooltip-enabled btn-edit-cat" data-toggle="tooltip" title="" data-original-title="Edit">
                                    <i class="fa fa-pencil-alt"></i>
                                </button>
                                <button type="button" data-id="<?php echo $d->id;?>" class="btn btn-sm btn-primary js-tooltip-enabled btn-delete" data-toggle="tooltip" title="" data-original-title="Delete">
        ...
        <div class="modal-content">
            <div class="block block-themed block-transparent mb-0">
                <div class="block-header bg-primary-dark">
                    <h3 class="block-title">Thêm phòng ban</h3>
                ...
                <div class="block-content">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="dm-post-add-title">Tên phòng ban</label>
                            <input type="text" class="form-control" id="depart-name" name="depart-name" placeholder="Tên phòng ban">
                        </div>
                    </div>
                </div>
                </div>
...
<div class="modal" id="edit-meta-modal" tabindex="-1" aria-labelledby="modal-block-large" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="block block-themed block-transparent mb-0">
                <div class="block-header bg-primary-dark">
                    <h3 class="block-title">Phân quyền</h3>
                    <div class="block-options">
                        <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                            <i class="fa fa-fw fa-times"></i>
                        </button>
