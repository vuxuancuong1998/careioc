...
Class member{
	...
	public function get_xid_by_username($username)
	{
		global $db;
		$member = $db->query("SELECT xid FROM xdata_account WHERE username = '".$username."' LIMIT 1");
		if($db->num_row())
		{
			return $db->fetch_object($first_row = true)->xid;
		}
		else
		{
			return 0;
	...
	public function get_member_fullname($xid)
    {
        global $db;
        $blog = $db->query("SELECT firstname,name FROM xdata_info WHERE xid='".$xid."'");
	...
	public function count_friend($xid)
    {
        global $db;
        $db->query("SELECT count(*) as total FROM xdata_friend WHERE xid1 ='".$xid."' OR xid2 = '".$xid."'");
        return ($db->fetch_object(true)->total)+0;
    }
	public function count_medal($xid)
	{
		global $db;
        $db->query("SELECT count(*) as total FROM xdata_medal WHERE xid ='".$xid."'");
        return ($db->fetch_object(true)->total)+0;
	}
